...
try:
    ...
    inputs = processor(
        text="<|image|>What is in this image?", images=image, return_tensors="pt"
    )
...
try:
    ...
    for size in test_sizes:
        ...
        try:
            ...
            inputs = processor(
                text="<|image|>What is this?", images=resized_image, return_tensors="pt"
            )

            print(
                f"  Input shapes: {inputs['input_ids'].shape}, pixel_values: {inputs.get('pixel_values', 'N/A')}"
            )

            # Try generation with minimal config
            inputs = {
                k: v.to("cuda:0") if hasattr(v, "to") else v for k, v in inputs.items()
            }
