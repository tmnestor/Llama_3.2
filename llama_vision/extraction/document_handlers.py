...
class DocumentTypeHandler(ABC):
    ...
    def classify(self, ocr_text: str, base_confidence: float = 0.5) -> ClassificationResult:
        ...
        found_indicators = [
            ...
            if indicator.lower() in ocr_lower
        ]
        ...
        is_match = (
            len(found_indicators) >= 2 or  # At least 2 indicators
            len(pattern_matches) >= 1 or   # At least 1 pattern match
            indicator_ratio >= 0.5         # At least 50% of indicators
        )
    ...
    def extract_fields(self, response: str) -> ExtractionResult:
        ...
        compliance_score = required_found / len(required_patterns) if required_patterns else 1.0
...
class DocumentTypeRegistry:
    ...
    def register_handler(self, handler: DocumentTypeHandler) -> None:
        ...
        self.logger.info(f"Registered handler for document type: {handler.document_type}")
